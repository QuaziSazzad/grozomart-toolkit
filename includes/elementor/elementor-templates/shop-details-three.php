                    ...
                    <div class="col-lg-6">
                        <div class="shop-details-simple-content">
                            ...
                            if ('yes' === $settings['layout_three_show_wishlist_row']) :
                                 ...
                                 * (compare at 20, wishlist at 25) — a hook meant for exactly this
                                 * spot — and each renders a real button carrying the classes,
                                 * product id and nonce its AJAX handlers expect. Firing the hook
                                 * gives working buttons that reflect whether the product is
                                 * already listed; a plain <a> to the wishlist page only navigated
                                 * away without ever adding anything.
                                 *
                                 * Nothing renders when both modules are off, which is the
                                 * intended behaviour — so the row is only output when it has
                                 * content.
                                 */
                                ob_start();
                                do_action('storzen_product_single_after_price', $details_product);
                                $details_actions = trim(ob_get_clean());

                                if ('' !== $details_actions) :
                            ?>
                                    <div class="add-list-items">
                                        <?php echo $details_actions; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Storzen's own button markup.
                                        ?>
                                    </div>
                            <?php
                                endif;
